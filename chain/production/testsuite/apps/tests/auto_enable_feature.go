// chain/production/testsuite/apps/tests/auto_enable_feature.go
package main
...
func Test_Auto_Enable() {
	...
	}, func(ne *devframework.NodeEngine) {})
	...
	for {
		...
		if height > 600 {
			accounts := make([]account.Account, len(node.GetAllAccounts()))
			for i, acc := range node.GetAllAccounts() {
				accounts[i] = *acc
			}
			node.SendFeatureStat(accounts, []string{"TestFeature_2", "TestFeature"})
			node.SendFinishSync(stakers, 0)
			node.SendFinishSync(stakers, 1)
		}
		// epoch := currentBeaconBlock.GetCurrentEpoch()
		...
		if height > 800 {
			...
			node.SendFeatureStat(accounts, []string{"TestFeature_3", "TestFeature"})
