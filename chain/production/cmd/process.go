// chain/production/cmd/process.go
package main
...
var (
	ChainMainParam   = config.MainnetParam
	ChainTestParam   = config.Testnet1Param
)
	...
	case backupChain:
		{
			...
			if cfg.ShardIDs != "" {
				...
				if cfg.ShardIDs == "all" {
					...
					if cfg.TestNet {
						numberOfShards = ChainTestParam.ActiveShards
					} else {
						numberOfShards = ChainMainParam.ActiveShards
					}
