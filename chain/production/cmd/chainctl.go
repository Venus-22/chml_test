// chain/production/cmd/chainctl.go
package main
...
func makeBlockChain(databaseDir string, testNet bool) (*blockchain.BlockChain, error) {
	...
	txPool.Init(&mempool.Config{
		PubSubManager: pb,
		DataBase:      map[int]incdb.Database{0: db}, // Correct the type of DataBase
		BlockChain:    bc,
		FeeEstimator:  make(map[byte]*mempool.FeeEstimator), // Correct type for FeeEstimator
	})

	// Remove bcParams since it's not used or declared
