...
func (engine *Engine) loadKeysFromMiningKey() {
	...
	if len(keys) == 0 {
		log.Printf("No mining keys provided in the configuration.")
		return
	}
	...
	for _, key := range keys {
		if key == "" {
			log.Printf("Empty mining key detected, skipping...")
			continue
		}

		log.Printf("Attempting to load mining key: %s", key)
		miningKey, err := GetMiningKeyFromPrivateSeed(key)
		if err != nil {
			log.Printf("Failed to load mining key: %s, error: %v", key, err)
	...
	if len(engine.validators) == 0 {
		log.Printf("No valid mining keys could be loaded. Consensus cannot proceed.")
		return
	}

	// Restrict to one key (as per original logic)
	engine.validators = engine.validators[:1]
	log.Printf("Using the first valid mining key: %s", engine.validators[0].PrivateSeed)

	// Set monitor public keys
	pubkeys := []string{}
	for _, val := range engine.validators {
		pubkey := val.MiningKey.GetPublicKey().GetMiningKeyBase58("bls")
		pubkeys = append(pubkeys, pubkey)
		log.Printf("Mining public key added for monitoring: %s", pubkey)
	}

	monitor.SetGlobalParam("MINING_PUBKEY", strings.Join(pubkeys, ","))
	log.Printf("Mining public keys successfully set for monitoring.")
}
