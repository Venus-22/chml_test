// chain/production/mempool/mempool.go
package mempool
...
func hasCommitteeRelatedTx(txs ...metadata.Transaction) bool {
	for _, tx := range txs {
		if tx.GetMetadata() != nil {
			...
			case metadata.BeaconStakingMeta, metadata.ShardStakingMeta, metadata.StopAutoStakingMeta, metadata.UnStakingMeta, metadata.AddStakingMeta, metadata.ReDelegateMeta:
				return true
			}
		}
	}
